"use client"
import React, { useEffect } from "react"
import { useState } from "react"
import axios from "axios"
import { useAuth } from "@/components/AuthContext"
import Navbar from "./Navbar"
import Hero from "./Hero"

export default function ChangePasswordPage() {
  const auth = useAuth()
  const accessToken = auth.getAccessToken()
  const [successMessage, setSuccessMessage] = useState<string | null>(null)
  const [userPasswordChange, setUserPasswordChange] = useState({
    currentPassword: "",
    newPassword: "",
    confirmationPassword: "",
  })

  const [errorMessage, setErrorMessage] = useState<string | null>(null)

  const onSignup = async () => {
    if (
      !userPasswordChange.currentPassword ||
      !userPasswordChange.newPassword ||
      !userPasswordChange.confirmationPassword
    ) {
      setErrorMessage("Fyll i alla fält")
      return
    }
    try {
      const response = await axios.patch(
        "http://localhost:5000/api/users/changepassword",
        {
          currentPassword: userPasswordChange.currentPassword,
          newPassword: userPasswordChange.newPassword,
          confirmationPassword: userPasswordChange.confirmationPassword,
        },
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
        }
      )
      if (response.status === 200) {
        setSuccessMessage("Lösenordet har ändrats!")
        console.log("Added successfully")
        // Reset
        setUserPasswordChange({
          currentPassword: "",
          newPassword: "",
          confirmationPassword: "",
        })
      } else {
        console.error("Invalid response format:", response)
        setErrorMessage("Ogiltigt svar från servern")
      }
    } catch (error: any) {
      console.log(
        "Register of password change failed, try again!!!!",
        error.message
      )
      setErrorMessage("Registrering of password change misslyckades")
    }
  }
  const closeErrorMessage = () => {
    setErrorMessage(null)
  }
  const closeSuccessMessage = () => {
    setSuccessMessage(null)
  }

  // Parse the token payload
  const tokenPayload = accessToken
    ? JSON.parse(atob(accessToken.split(".")[1]))
    : null
  const username = tokenPayload ? tokenPayload.Username.toUpperCase() : null

  return (
    <div className="overflow-hidden">
      <Navbar />
      <Hero />
      <div className="home__profile-container">
        {username && (
          <h2 className="text-[#41bd47] text-xl font-bold">
            HÄR KAN DU BYTA DITT LÖSENORD {username}
          </h2>
        )}
      </div>
      <div>
        <section className="relative z-10 pt-30 flex-col flex justify-center items-center h-full w-full">
          <div className="flex h-full items-center justify-center lg:justify-between">
            <div className="w-full h-full flex-wrap items-center justify-center lg:justify-between">
              <div className="mt-10 mb-12 w-full h-full flex-wrap items-center justify-center lg:justify-between">
                <form>
                  {/* currentPassword input */}
                  <div className="relative mb-6">
                    <input
                      type="text"
                      className="text-[#41bd47] peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[2.15] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none shadow-xl"
                      id="currentPassword"
                      placeholder=""
                      value={userPasswordChange.currentPassword}
                      onChange={(e) =>
                        setUserPasswordChange({
                          ...userPasswordChange,
                          currentPassword: e.target.value,
                        })
                      }
                    />
                    <label
                      htmlFor="currentPassword"
                      className={`pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[2.15] text-[#41bd47] transition-all duration-200 ease-out ${
                        userPasswordChange.currentPassword
                          ? "text-[#41bd47] -translate-y-[1.15rem] scale-[0.8]"
                          : ""
                      }`}
                    >
                      Nuvarande lösenord
                    </label>
                  </div>

                  {/* newPassword input */}
                  <div className="relative mb-6">
                    <input
                      type="text"
                      className="text-[#41bd47] peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[2.15] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none shadow-xl"
                      id="newPassword"
                      placeholder=""
                      value={userPasswordChange.newPassword}
                      onChange={(e) =>
                        setUserPasswordChange({
                          ...userPasswordChange,
                          newPassword: e.target.value,
                        })
                      }
                    />
                    <label
                      htmlFor="newPassword"
                      className={`pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[2.15] text-[#41bd47] transition-all duration-200 ease-out ${
                        userPasswordChange.newPassword
                          ? "text-[#41bd47] -translate-y-[1.15rem] scale-[0.8]"
                          : ""
                      }`}
                    >
                      Nytt lösenord
                    </label>
                  </div>

                  {/* confirmationPassword input */}
                  <div className="relative mb-6">
                    <input
                      type="confirmationPassword"
                      className="text-[#41bd47] peer block min-h-[auto] w-full rounded border-0 bg-transparent px-3 py-[0.32rem] leading-[2.15] outline-none transition-all duration-200 ease-linear focus:placeholder:opacity-100 data-[te-input-state-active]:placeholder:opacity-100 motion-reduce:transition-none shadow-xl"
                      id="confirmationPassword"
                      placeholder=""
                      value={userPasswordChange.confirmationPassword}
                      onChange={(e) =>
                        setUserPasswordChange({
                          ...userPasswordChange,
                          confirmationPassword: e.target.value,
                        })
                      }
                    />
                    <label
                      htmlFor="password"
                      className={`pointer-events-none absolute left-3 top-0 mb-0 max-w-[90%] origin-[0_0] truncate pt-[0.37rem] leading-[2.15] text-[#41bd47] transition-all duration-200 ease-out ${
                        userPasswordChange.confirmationPassword
                          ? "text-[#41bd47] -translate-y-[1.15rem] scale-[0.8]"
                          : ""
                      }`}
                    >
                      Konfirmera lösenord
                    </label>
                  </div>

                  {/* Login button */}
                  <div className="text-center lg:text-left">
                    <button
                      title="Sign Up"
                      type="button"
                      className="text-[#41bd47] inline-block rounded px-7 pb-2.5 pt-3 text-sm shadow-xl transition duration-150 ease-in-out"
                      onClick={onSignup}
                    >
                      Registrera
                    </button>
                  </div>

                  {/* Success message popup */}
                  {successMessage && (
                    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-[#41bd47] bg-opacity-50">
                      <div className="bg-white p-4 rounded shadow-md">
                        <p>{successMessage}</p>
                        <button
                          onClick={closeSuccessMessage}
                          className="mt-4 text-sm text-white bg-[#41bd47] px-4 py-2 rounded"
                        >
                          Stäng
                        </button>
                      </div>
                    </div>
                  )}

                  {/* Error message popup */}
                  {errorMessage && (
                    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-800 bg-opacity-50">
                      <div className="bg-white p-4 rounded shadow-md">
                        <p>{errorMessage}</p>
                        <button
                          onClick={closeErrorMessage}
                          className="mt-4 text-sm text-white bg-gray-800  px-4 py-2 rounded"
                        >
                          Stäng
                        </button>
                      </div>
                    </div>
                  )}
                </form>
              </div>
            </div>
          </div>
        </section>
      </div>
    </div>
  )
}
